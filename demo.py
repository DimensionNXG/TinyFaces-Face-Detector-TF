...
for video in glob.glob (VideoDirectoryPath + "/*.webm") :
    ...
    # ret, img = cap.read()
    #fetching dimensions of video
    frame_width = int(cap.get(3))
    frame_height = int(cap.get(4))
    #Video writer instantiate
    out = cv2.VideoWriter (str(video)+'_Output.avi', cv2.VideoWriter_fourcc ('M', 'J', 'P', 'G'), 10, (frame_width,frame_height))
    while (cap.isOpened ()) :
        ret, frame = cap.read ()
        try :
            ...
            if ret == True :
                #writing frames to out
                out.write (output)

                # Display the resulting frame
                #turning off Imshows and waitkeys for video writing
